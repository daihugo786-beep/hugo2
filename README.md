...
免費額度（依模型不同，實際以 Google 當下公告為準）：`gemini-2.0-flash-lite` 大約每分鐘 30 次、每天 1500 次請求；這個工具每次提問會用到 2 次請求（先規劃查詢、再寫摘要），大約等於每天可以問 700 多次，個人或小型分享用途綽綽有餘。
...
| `GEMINI_MODEL` | （選填）預設 `gemini-2.0-flash-lite`（額度最高、速度最快）。想要理解力更好可以改成 `gemini-2.5-flash`（額度較少，每天約500次請求） |
...
內建 2003–2026 年、22 縣市（已合併升格前後行政區）的施政滿意度資料，共 552 筆。資料口徑說明（計分方式變動、縣市合併等）已內建在網頁裡。
